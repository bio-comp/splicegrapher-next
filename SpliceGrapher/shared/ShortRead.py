...
def depthsToClusters(chromosome, depths, **args):
    ...
    while maxpos > minpos > 0 and depths[minpos] >= threshold:
        minpos -= 1
    while minpos < maxpos < len(depths) and depths[maxpos] >= threshold:
        maxpos += 1
    ...
    if current and current.avgDepth() >= minDepth:
        result.append(current)
    return result
...
class Read(object):
    ...
    def __repr__(self):
        return "%d reads on %s from %d to %d (%s)" % (
            self.count,
            self.chromosome,
            self.p1,
            self.p2,
    ...
    def sitesMatch(self, other):
        return (
            ...
            and other.accval == self.accval
            and other.donval == self.donval
        )
...
class SplicedRead(Read):
    ...
    def __repr__(self):
        return "%d spliced reads on %s from %d-%d to %d-%d (%s)" % (
            self.count,
            self.chromosome,
            self.p1,
            self.p2,
...
class Cluster(object):
    ...
    def __repr__(self):
        return "%s: %s %d-%d (%d nt) avg. depth %.2f" % (
            self.id,
            self.chromosome,
            self.minpos,
            self.maxpos,
